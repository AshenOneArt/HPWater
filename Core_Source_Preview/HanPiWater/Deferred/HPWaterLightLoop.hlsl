#ifndef HPWATER_LIGHT_LOOP_HLSL
#define HPWATER_LIGHT_LOOP_HLSL
...
IndirectLighting HPWaterEvaluateBSDF_Env(  LightLoopContext lightLoopContext,
...
uint featureFlags,out LightLoopOutput lightLoopOutput)
{
    // Init LightLoop output structure
    ZERO_INITIALIZE(LightLoopOutput, lightLoopOutput);
    WaterLightLoopData waterLightLoopData;
    ZERO_INITIALIZE(WaterLightLoopData, waterLightLoopData);
    ...
    //将采样Shadow挪至waterBSDF当中，如果是体积延迟光照

    //如果是镜面反射光照，则需要计算阴影
#if defined(SPECULAR_LIGHTING_ON)    
    ...
    #endif
                        context.shadowValue = WaterVolumeLightLoop::GetWaterDirectionalShadowAttenuation(context.shadowContext,
                                                                            posInput.positionSS, positionWS, GetNormalForShadowBias(bsdfData),
                                                                            light.shadowIndex, L,1);     
                                                                            
    #ifdef LIGHT_EVALUATION_SPLINE_SHADOW_VISIBILITY_SAMPLE
                        // Tap the shadow a second time for strand visibility term.
                        context.splineVisibility = WaterVolumeLightLoop::GetWaterDirectionalShadowAttenuation(context.shadowContext,
                                                                                posInput.positionSS, posInput.positionWS, GetNormalForShadowBias(bsdfData),
                                                                                light.shadowIndex, L,1);
    #endif
                    }
...
#endif
// 使用低分辨率坐标生成噪声，确保每个低分辨率像素都有独特的噪声值
// 如果使用 posInput.positionSS（全分辨率），相邻低分辨率像素可能使用相同噪声
#if defined(NEED_LOWRES_POSITIONINPUT)
    float  dither = GenerateRandom1DOffset(_HPWaterLowResPositionSS, 1, GetTaaFrameInfo().z); 
#else
    ...
    // 没有折射
        ...
        int2 coord = int2(posInput.positionNDC.xy * _ScreenSize.xy);
    ...
    waterLightLoopData.RefractWaterScreenCoord = RefractWaterScreenCoord;
    waterLightLoopData.RelativeRefractedEndPos = refractedBackWorldPos;
    ...
    if (featureFlags & LIGHTFEATUREFLAGS_PUNCTUAL)
    {
        ...
        //先关闭除定向光以外的其他光源，这块的逻辑在体渲染中很难处理，后面再来想想怎么做
/* #if NEED_TO_CHECK_HELPER_LANE
    ...
    //这里后面需要处理一下水下，参考Water.hlsl中的EvaluateBSDF_Env
#define EVALUATE_BSDF_ENV_SKY(envLightData, TYPE, type) \
        IndirectLighting lighting = HPWaterEvaluateBSDF_Env(context, V, posInput, preLightData, envLightData, bsdfData, envLightData.influenceShapeType, MERGE_NAME(GPUIMAGEBASEDLIGHTINGTYPE_, TYPE), MERGE_NAME(type, HierarchyWeight)); \
...
#define EVALUATE_BSDF_ENV(envLightData, TYPE, type) if (IsMatchingLightLayer(envLightData.lightLayers, builtinData.renderingLayers)) { EVALUATE_BSDF_ENV_SKY(envLightData, TYPE, type) }
    ...
    // First loop iteration (折射功能已禁用，仅保留反射)
    if (featureFlags & (LIGHTFEATUREFLAGS_ENV | LIGHTFEATUREFLAGS_SKY | LIGHTFEATUREFLAGS_SSREFLECTION))
    {
...
// PROBE_VOLUMES support removed to reduce shader variants
...
#endif
            {
// PROBE_VOLUMES support removed to reduce shader variants - fallback to ambient probe
/* #if defined(PROBE_VOLUMES_L1) || defined(PROBE_VOLUMES_L2)
...
#endif
            {
                ...
                if (s_envLightIdx >= v_envLightIdx)
                {
                    ...
                    if (reflectionHierarchyWeight < 1.0)
                    {
                        ...
                        {
                            IndirectLighting lighting = HPWaterEvaluateBSDF_Env(context, V, posInput, preLightData, s_envLightData, bsdfData, s_envLightData.influenceShapeType, GPUIMAGEBASEDLIGHTINGTYPE_REFLECTION, reflectionHierarchyWeight);
// PROBE_VOLUMES support removed to reduce shader variants - APV normalization disabled
/* #if defined(PROBE_VOLUMES_L1) || defined(PROBE_VOLUMES_L2)
...
#undef EVALUATE_BSDF_ENV_SKY
    ...
    if (featureFlags & LIGHTFEATUREFLAGS_DIRECTIONAL)
    {
        for (i = 0; i < _DirectionalLightCount; ++i)
        {
            ...
            {
#if defined(HP_WATER_VOLUME)
                DirectLighting lighting = WaterVolumeLightLoop::WaterEvaluateBSDF_Directional(waterLightLoopData,featureFlags,context, V, posInput, preLightData, _DirectionalLightDatas[i], bsdfData, builtinData);
                AccumulateDirectLighting(lighting, aggregateLighting);
#elif defined(SPECULAR_LIGHTING_ON)
                DirectLighting lighting = WaterSpecularLightLoop::WaterEvaluateBSDF_Directional(waterLightLoopData,featureFlags,context, V, posInput, preLightData, _DirectionalLightDatas[i], bsdfData, builtinData);
                AccumulateDirectLighting(lighting, aggregateLighting);
#endif
            }
        }
